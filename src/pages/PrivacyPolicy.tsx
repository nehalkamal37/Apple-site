...
<Section className="bg-gradient-to-br from-gray-50 to-gray-100 py-12 px-4 sm:px-6">
  <motion.div
  ...
  >
    ...
    <motion.div
      ...
      className="relative bg-white/90 backdrop-blur-md rounded-xl shadow-md p-4 sm:p-6 border border-gray-200"
    >
      <button
        onClick={() => setShowTOC(!showTOC)}
        className="flex flex-wrap items-center text-base sm:text-lg font-semibold text-gray-800 hover:text-primary-600"
      >
        <ClipboardList className="mr-2 text-primary-600 w-5 h-5" />
        {showTOC ? 'Hide Table of Contents' : 'Show Table of Contents'}
      </button>
      {showTOC && (
        <ul className="mt-4 space-y-2 break-words text-sm sm:text-base">
          {tocItems.map((item) => (
            <li key={item.id}>
              <a
                href={item.id}
                className="flex flex-wrap items-center text-gray-700 hover:text-primary-600 transition-colors break-all"
              >
      ...
      <div className="p-4 sm:p-6 md:p-8 space-y-10">
        ...
        <SectionBlock id="your-rights" title="Your Rights" icon={Lock} content={[
              ...
              <SectionBlock id="file-complaint" title="File a Complaint" icon={Mail} content={[
                "Contact our Privacy Officer at requests@applespecialtypharmacy.com or (323) 999-2775.",
        ...
        <section id="contact-us" className="scroll-mt-20 space-y-6">
          <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-800 mb-4 flex items-center break-words">
          ...
          <p className="text-gray-700 text-base sm:text-lg break-words">
            To exercise your rights or for more information, contact our Privacy Officer:
          </p>

          <ul className="ml-6 space-y-3 text-gray-700 list-disc text-sm sm:text-base break-words">
            <li className="break-words">
              Email:{' '}
              <a
                href="mailto:requests@applespecialtypharmacy.com"
            ...
            <li className="break-words">
              Phone:{' '}
              <a href="tel:3239992775" className="text-primary-600 underline">
                323-999-2775
              </a>
            </li>
            <li className="break-words">
              Mail: Apple Specialty Pharmacy, 1211 N. Broadway, Ste 300, Santa Ana, CA 92701
            </li>
          </ul>
        </section>
      </div>
...
{/*
      ...
      <Mail className="h-6 w-6 text-primary-600" />
                ...
                <ul className="ml-6 space-y-3 text-gray-700 list-disc">
                  <li>
                    <span role="img" aria-label="email"></span> Email: <a href="mailto:Requests@applespecilatypharmacy.com" className="text-primary-600 underline">requests@applespecialtypharmacy.com</a>
                  </li>
                  <li>
  ...
  <section id={id} className="scroll-mt-20">
    ...
    <ul className="ml-12 space-y-2 text-gray-700 list-disc">
      {content.map((text, idx) => (
        <li key={idx} className="break-words break-all whitespace-normal">{text}</li>
